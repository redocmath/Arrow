...
def parse_line(word):
    ...
    for i in range(len(word)):
        try:
            if word[i] == '=':
                if word[0] != 'function' and word[i + 1] != 'list' and re.fullmatch(r'^[a-z]+\w*_*\d*', word[0]):
                    string = word[i + 1]
                    ...
                    try:
                        ...
                        pass
                    list_word = word[0].split('[')
                    list_word[1] = list_word[1].split(']')[0]
                    if len(list_word) == 2:
                        list_word[1] = list_word[1].replace(" ", "")
                        try:
                            for v in range(len(var_key)):
                                for k in range(len(var_key)):
                                    list_word[1] = list_word[1].replace(var_key[k], str(var_value[k]))
                                    if debug:
                                        print('var = ', list_word[1])
                        except:
                            pass
                        var[list_word[0] + '[' + str(list_word[1]) + ']'] = string
                        pass
                elif word[i + 1] == 'list':
                    for j in range(int(word[i + 2])):
                        var[word[i - 1] + '[' + str(j) + ']'] = 0
                var_value = list(var.values())
                var_key = list(var.keys())
                break
                    ...
                    if parameter[j] in part[2]:
                        try:
                            if real_parameter[j][0] == "'" and real_parameter[j][len(real_parameter[j]) - 1] == "'":
                                ...
                                for a in range(len(var)):
                                    if var_key[a] in edit:
                                        edit = edit.replace(str(var_key[a]), str(var_value[a]))
                                edit = eval(edit)
                                print(edit, end=' ')
                        except:
                            continue
                        ...
                        if word[0] != 'function' and word[i + 1] != 'list' and re.fullmatch(r'^[a-z]+\w*_*\d*', word[0]):
                            ...
                            if len(list_word) == 2:
                                ...
                                try:
                                    for v in range(len(var_key)):
                                        for k in range(len(var_key)):
                                            list_word[1] = string.replace(var_key[k], var_value[k])
                                            if debug:
                                                print('var = ', list_word[1])
                                except:
                                    pass
